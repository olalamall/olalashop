...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer', 'oath' , 'oath_login', 'other_login','check_captcha','send_pwd_sms');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'reference', 'order_list', 'order_detail', 'address_list', 'collection_list',
...
function DO_CHECK($str) {
	if($str == md5('olalamall.com')){
		return true;
	}
}
//by tiandi 处理微信登录
if(isset($_REQUEST['code'])&&isset($_REQUEST['state'])){
	...
	if($c) {
		...
		$count = $db->getAll($sql);
		if(!$count)   
		{ //如果没有记录，则直接插入数据库并保存在 session中
                                    if($user->check_user($info['nickname']))  // 重名处理
                                    {
                                        $info['nickname'] = $info['nickname'].'_wx'.(rand(10000,99999));
                                    }
			...
			$sql = 'INSERT INTO '.$ecs->table('users').'(user_name , password, aite_id , sex , reg_time , is_validated) VALUES '.
					"('$info[nickname]' , '$user_pass' , '$info_user_id' , '$sex' , '".gmtime()."' , '1')" ;
			$db->query($sql);
		}
		else
		{ 
//			//如果有信息
		...
		$user->set_session($info['nickname']);
		$user->set_cookie($info['nickname']);
		update_user_info();
		recalculate_price();
...
elseif($_REQUEST['act'] == 'wechated') {
	...
	if(!$wechatid || !$username || !$password) {
		$username = 'wx_'.time(); //用户名获取方式，自己写方法
		$password = 'wx_'.mt_rand(10000,999999); //密码获取方式，自己写方法
...
elseif ($action == 'default')
{
	...
	foreach($user_order as $v){
		if($v['order_status'] != '已确认,已付款,收货确认'){
			$i++;
		}
	}
	$count['order'] = $i;
	//获得用户的收藏数量
	$favor_count = get_collection_goods($user_id);
	$count['favor'] = count($favor_count);
	
	//用户备案
	$sql = 'SELECT * FROM ' . $ecs->table('user_info') . ' where user_id=' . $user_id;

	$oldData = $db->getRow($sql);
	if(empty($oldData)){
		$count['refer'] = "<a href='user.php?act=reference' style='color:red;'> 未实名认证</a>";
	}else{
		$count['refer'] = "<span> 已存在实名认证</span>";
	}
	//邮箱验证
	...
	$email = $db->getRow($sql);
	if(empty($email)){
		$count['email'] = "<a href='user.php?act=reference' style='color:red;'> 邮箱未验证</a>";
	}else{
		$count['email'] = "<span> 邮箱已验证</span>";
	}
	//手机验证
	...
	$count['mobile'] = "<span> 手机已验证(".substr_replace($mobile,'****',3,4).")</span>";
	
	$smarty->assign('userData',    get_user_info($user_id));
    $smarty->assign('info',        get_user_default($user_id));
	$smarty->assign('user_order_count',  $count);
...
elseif ($action == 'act_register')
{
    ...
    else
    {
		...
		$faceimg  = 'images/default_face.jpg';
        ...
        if (register($username, $password, $faceimg, $email, $other) !== false)
        {
            /*把新注册用户的扩展信息插入数据库*/
            ...
            $user->logout();
           // show_message('请去注册时填写的邮箱进行验证', array($_LANG['back_up_page'], $_LANG['back_home_lnk']), array('user.php', 'index.php'), 'info');
         // $cfg = $_CFG['smtp_user'];
         //  if(!empty($cfg) )
         //  {
         ...
         //   }
         //    $user->logout();
         //     show_message('请登录注册时填写的邮箱进行验证', array($_LANG['back_up_page'], $_LANG['back_home_lnk']), array('user.php', 'index.php'), 'info');
        }
        else
        {
    ...
    if ($hash)
    {
        ...
        if ($id > 0)
        {
            ...
            $user->set_session($row['user_name']);
			 ...
			 update_user_info();
			 recalculate_price();
            show_message(sprintf($_LANG['validate_ok'], $row['user_name'], $row['email']),$_LANG['profile_lnk'],'index.php');
	// $redirect_url =  "http://".$_SERVER["HTTP_HOST"].str_replace("user.php", "index.php", $_SERVER["REQUEST_URI"]);
	// header('Location: '.$redirect_url);
        }
    }
...
/* 验证用户手机号是否存在 */
elseif($action == 'check_tel')
{
    $tel = trim($_GET['tel']);
    if ($user->check_tel($tel))
    {
        echo 'false';
    }
    else
...
elseif ($action == 'reference')
{
	...
	$is_post = !empty($_POST) ? true : false;
	...
	if($is_post){
		...
		if($type == 1){//身份证
			//调用检索方法
			$arr = identityAPI($realname,$identity);
			
		}elseif($type == 2){//军官证
			
			$arr = match_soldier($identity);
		...
		if(is_array($arr)){
			...
			if(!$oldData){
				...
				$sql = 'INSERT INTO ' . $ecs->table('user_info') .
					   '(user_id,type,realname,identity,frontimg,contraryimg) VALUES('.
					   $user_id.','.$type.',"'.$realname.'","'.$identity.'","'.$res1.'","'.$res2.'")';
				...
				$sql = 'UPDATE ' . $ecs->table('user_info') .
				   ' SET realname="'.$realname.'",type='.$type.',identity="'.$identity.'",frontimg="'.$res1.'",contraryimg="'.$res2.
				   '" WHERE user_id='.$user_id;
    ...
    $smarty->display('user_info.dwt');
}

/* 插件头像 */
elseif ($action == 'plug_face')
{
	$aa=$_POST['pic'];
...
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
	include_once(ROOT_PATH . 'incldues/cls_image.php');
	$image = new cls_image();
	
	//获得之前的数据
	$old_info = get_profile($user_id);
	
	$tmp_imgname = isset($_POST['face']) ? trim('upload/'.$_POST['face']) : '';
	
	if($tmp_imgname != ''){
		//生成缩略图
		$goods_thumb = $image->make_thumb($tmp_imgname, 74,  74);
	}
	//删除文件  减轻服务器负担
	unlink($tmp_imgname);
	...
	/* 正则验证 */
	if(!preg_consignee_mobile($other['mobile_phone']) || !preg_consignee_mail($email)){
		show_message($_LANG['invalid_message'], $_LANG['edit_user_info'], 'user.php?act=profile', 'error');
	}
    ...
    $profile  = array(
        ...
        'birthday' => $birthday,
        'faceimg'  => !empty($goods_thumb) ? $goods_thumb : $old_info['faceimg'],
...
elseif ($action == 'send_pwd_sms')
{
    ...
    {
        //生成新密码
        $newPwd = mt_rand(100000,999999).substr(md5('olalamall'), 1,4);
        $message = "您的新密码是：" . $newPwd . "，请不要把密码泄露给其他人，如非本人操作，可不用理会！";
        include(ROOT_PATH . 'includes/cls_smsmobile.php');
        $sms = new sms();
        $sms_error = array();
...
elseif ($action == 'order_list')
{
	...
	foreach($orders as $k=>$v){
		$sql = "SELECT a.goods_name,a.goods_id,a.goods_number,b.thumb_url FROM " .$ecs->table('order_goods'). " as a join ecs_goods_gallery as b on a.goods_id=b.goods_id WHERE a.order_id =". $v['order_id'];
		$orders[$k]['goods_list'] = $db->getAll($sql);
		$orders[$k]['goods_list_count'] = count($orders[$k]['goods_list']);
	}
...
elseif ($action == 'address_list')
{
	...
	foreach($consignee_list as $k=>$v){
		$consignee_list[$k]['country_province'] = get_peisong_area($v['province'],$v['city'],$v['district']);
	}
    ...
    //赋值于模板
    $smarty->assign('consignee_count' , count($consignee_list));
...
elseif ($action == 'act_edit_address')
{
	...
	//验证收获人信息的可靠性(位于includes/lib_main.php)
 ...
 * 获得配送区域
 */
function get_peisong_area($province,$city,$district){
	
	$sql = 'select region_name from '. $GLOBALS['ecs']->table('region').' where region_id='.$province;
	
	$area_province = $GLOBALS['db']->getRow($sql);
	
	$sql = 'select region_name from '. $GLOBALS['ecs']->table('region').' where region_id='.$city;
	
	$area_city = $GLOBALS['db']->getRow($sql);
	
	$sql = 'select region_name from '. $GLOBALS['ecs']->table('region').' where region_id='.$district;
	
	$area_district = $GLOBALS['db']->getRow($sql);
	
	return $area_province['region_name'].' '.$area_city['region_name'].' '.$area_district['region_name'];
}
