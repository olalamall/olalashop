...
function group_buy_list($size, $page)
{
	...
	foreach($arr as $v){
		$sql = 'UPDATE '.$GLOBALS['ecs']->table('goods').' SET is_on_sale=1 WHERE goods_id='.$v['goods_id'];
	...
	$sql = "SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb, b.act_id AS group_buy_id, g.market_price,".
            ...
            "FROM " . $GLOBALS['ecs']->table('goods_activity') . " AS b " .
                "LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON b.goods_id = g.goods_id " .
            "WHERE b.act_type = '" . GAT_GROUP_BUY . "' " .
            "AND b.start_time > '$now' OR b.is_finished >= 3 ORDER BY b.act_id DESC";
	$result = $GLOBALS['db']->getAll($sql);
	
	/* 下架商品 */
	foreach($result as $k=>$v){
		$sql = 'UPDATE '.$GLOBALS['ecs']->table('goods').' SET is_on_sale=0 WHERE goods_id='.$v['goods_id'];
		$GLOBALS['db']->query($sql);
	}
