...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
    ...
    foreach ($banjia as $k=>$v) {
            $banjia[$k]['double_price'] = '￥'.$v['shop_price']*2;
            $banjia[$k]['url'] = 'goods.php?id='.$v['goods_id'];
    }
    $smarty->assign('now_sale', $banjia); //今日新品
    
    $newGroup = index_group_list();
   
	$smarty->assign('newGroup',$newGroup);// 团购商品
	...
	/*分配一元海购数据*/
	$highgo = highgo_list();
	$smarty->assign('highgo',$highgo);
	
	/* 侧边栏分配购物车信息   */
	$cart = get_cart_goods();
	foreach($cart['goods_list'] as $v){
		$cart['total']['goods_number'] += $v['goods_number'];
	}
 ...
 * 
 */
function highgo_list(){
	//查询出highgo的数据
	$sql = 'select id,title,thumb from yy.go_shoplist where id=sid limit 5';
	
	$all = $GLOBALS['db']->getAll($sql);
	
	//压入goods表中的goods_name
	foreach($all as $k=>$v){
		$all[$k]['url'] = "http://yy.olalamall.com/?/goods/{$v['id']}";
	}
	//返回数据
	return $all;
}
...
$sql ="SELECT * FROM".$GLOBALS['ecs']->table('goods')."WHERE goods_id >= (SELECT floor( RAND() * ((SELECT MAX(goods_id) FROM".$GLOBALS['ecs']->table('goods').")-(SELECT MIN(goods_id) FROM".$GLOBALS['ecs']->table('goods').")) + (SELECT MIN(goods_id) FROM".$GLOBALS['ecs']->table('goods').")))  
    ORDER BY goods_id LIMIT 6";
    $res = $GLOBALS['db']->getAll($sql);
  ...
  * 自书写团购数据
  */
function index_group_list(){
	$time = gmtime();
	
	 //团购的时间
    $sql = 'SELECT a.*,b.thumb_url FROM ' . $GLOBALS['ecs']->table("goods_activity") . 'as a join ecs_goods_gallery as b '.
    		'on a.goods_id = b.goods_id and end_time >= '.$time.' WHERE act_type = 1';
    $group_arr = $GLOBALS['db']->getAll($sql);
	//新数组
	$newGroup = array();
	$newGroup['underway'] = array();
	$newGroup['expect'] = array();
	
	//获得当前抢购的抢购时间点
	$point_hours = floor(date('H',time())/4)*4;
	//向后推移时间点数组
	$expect = array();
	for($i=1;$i<=5;$i++){
		if($point_hours+4*$i >= 24){
			$expect[$i] = ($point_hours+4*$i)-24;
		}else{
			$expect[$i] = $point_hours+4*$i;
		}
		$newGroup['expect'][$expect[$i]] = array();
	}
	foreach($group_arr as $v){
		...
		if($v['is_finished'] == '0'){
			...
			//判断是否正在进行中
			$new_time = floor(date('H',($v['start_time']+8*3600))/4)*4;
			
			$stat = group_buy_stat($v['act_id'], $ext_info['deposit']);
			
			$row['cur_amount'] = $stat['valid_goods'];         // 当前数量
			
			$v['cur_amount'] = $row['cur_amount'];
			
			$v['url'] = 'group_buy.php?act=view&id='.$v['act_id'];
			
			$v['last_price'] = $ext_info['deposit'];
			
			if($new_time == $point_hours){//进行中的团购
				
				array_push($newGroup['underway'],$v);
				
			}else{//敬请期待
				if(in_array($new_time,$expect)){
					
					$newGroup['expect'][$new_time][]=$v;
