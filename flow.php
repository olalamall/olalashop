...
elseif ($_REQUEST['step'] == 'add_to_cart_showDiv')//点击添加到购物车弹出的div框
{
    include_once('includes/cls_json.php');
    $_POST['goods']=strip_tags(urldecode($_POST['goods']));
    $_POST['goods'] = json_str_iconv($_POST['goods']);
...
elseif ($_REQUEST['step'] == 'add_to_cart_combo') //by mike
{
    ...
    {
        $sql = "SELECT a.attr_id, a.attr_name, a.attr_type, ".
            "g.goods_attr_id, g.attr_value, g.attr_price, g.goods_weight, g.goods_tax " .
        'FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS g ' .
        'LEFT JOIN ' . $GLOBALS['ecs']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
        "WHERE a.attr_type != 0 AND g.goods_id = '" . $goods->goods_id . "' " .
        'ORDER BY a.sort_order, g.attr_price, g.goods_attr_id';
...
elseif ($_REQUEST['step'] == 'login')
{
    ...
    {
        $user->logout();
        show_message('请登录注册时填写的邮箱进行验证', array($_LANG['back_up_page'], $_LANG['back_home_lnk']), array('flow.php?step=login', 'index.php'), 'info');
      }
            }
            else
            {
...
elseif ($_REQUEST['step'] == 'consignee')//收货地址信息
{
    /*------------------------------------------------------ */
    //-- 收货人信息
    /*------------------------------------------------------ */
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
        ...
        /* 返回收货人页面代码 */
        $smarty->assign('real_goods_count', exist_real_goods(0) ? 1 : 0);
    }
    else
    {
    	//验证提交上来的收获人信息(位于includes/lib_main.php文件)
    	my_check_consignee_info($_POST);
        /*
         * 保存收货人信息
         */
        $consignee = array(
		...
		//判断是否存在备案信息如果不存在备案信息
		$sql = 'select * from '.$GLOBALS['ecs']->table('user_info').' where user_id='.$_SESSION['user_id'];
		$res = $GLOBALS['db']->getRow($sql);
		
		if(!is_array($res)){
			show_message('尚未填写备案信息!!!');
		}
...
elseif ($_REQUEST['step'] == 'checkout')//订单确认页 
{
    /*------------------------------------------------------ */
    //-- 订单确认
    /*------------------------------------------------------ */
	...
	/* 检查下单人备案信息是否完整 */
	if(!check_users_reference($_SESSION['user_id'])){
		/* 如果不完整则转向到收货人信息填写界面 */
        ecs_header("Location: flow.php?step=consignee\n");
        exit;
	}
	...
	foreach($cart_goods as $k=>$v){
		$sql = 'select goods_tax from'. $GLOBALS['ecs']->table('goods') .'where goods_id='.$v['goods_id'];
		$cart_goods[$k]['goods_tax'] = $db->getOne($sql);
		$weightsql = 'select goods_weight from'. $GLOBALS['ecs']->table('goods') .'where goods_id='.$v['goods_id'];
		$goods_weight = $db->getOne($weightsql);
		$cart_goods[$k]['goods_weight'] 	= $goods_weight;
		$cart_goods[$k]['subtotal_weight']	= $goods_weight*$v['goods_number'];
	}
//  echo '<pre>';
...
//	p($total);die;
	$last_one = '￥' . $total['zongjia'];
    $smarty->assign('total', $total);
    $smarty->assign('shopping_money', sprintf($_LANG['shopping_money'], $last_one));
    $smarty->assign('market_price_desc', sprintf($_LANG['than_market_price'], $total['formated_market_price'], $total['formated_saving'], $total['save_rate']));
	$smarty->assign('tax_price', $total['tax_total']);
	//正则匹配
	$total['formated_goods_price'] = preg_pipei($total['formated_goods_price']);
	
	
    $smarty->assign('total_price', $total['zongjia']);
...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   /* 判断是不是实体商品 */
	...
	foreach($cart_goods['goods_list'] as $value){
		$total['goods_tax'] += (float)$value['goods_tax'];
	}
	//如果税金小于50 则不计税金
//	if($total['goods_tax'] < 50){
//		$total['goods_tax'] = 0;
//	}
	
	/* 最终金额 = 市场价+总的税金 */
	
	$total['last_price'] = $total['goods_price'] + $total['goods_tax'];
	
    $order['bonus']        = $total['bonus'];
    $order['goods_amount'] = $total['last_price'];
    $order['discount']     = $total['discount'];
    $order['surplus']      = $total['surplus'];
    $order['tax']          = $total['tax'];
	$order['tax_fee']      = $total['tax_total'];
    ...
    /* 更新一元夺宝数据 */
	//1.检索购物车中是否有一元海购的商品，返回检索出的一元海购goods_id数组,如果没有检索出来则返回false
	$gids = match_highgo($order['order_id']);
	
	//2.如果有一元海购的商品则更新一元海购表中的current字段，并且减少后台库存
	if($gids){
		foreach($gids as $v){
			//获得订单中一元海购的数量
			$sql = 'select goods_number from ' . 
			...
			//获得一元海购表的当前人数
			$sql = 'select current from ' .
					$ecs->table('highgo') .
					' where goods_id = ' .
					$v . ' and status = 0';
			$current = $db->getOne($sql);
			$current += $res;
			
			//拿出总人数
			$sql = 'select headcount from ' .
					$ecs->table('highgo') .
					' where goods_id = ' .
					$v . ' and status = 0';
			$headcount = $db->getOne($sql);
			//更新到数据库
			$sql = "UPDATE ".$GLOBALS['ecs']->table('highgo').
                        " SET current = " . $current . " WHERE goods_id=" . $v . " and status = 0";
			$db->query($sql);
			
			//更新此商品的库存
			$inventory = $headcount-$current;
			//判断计算的库存小于0  则库存为0
			if($inventory < 0){
				$inventory = 0;
			}
			$sql = "UPDATE ".$GLOBALS['ecs']->table('goods').
					" SET goods_number = " . $inventory . " where goods_id=" .$v;

			$db->query($sql);
			//判断如果总和达到总人数则改变状态
			if($current >= $headcount){
				$sql = "UPDATE ".$GLOBALS['ecs']->table('highgo').
                        " SET status = 1 WHERE goods_id=" . $v . " and status = 0";
				$db->query($sql);
			}
    ...
    {
    	//加上税金
    	$order['order_amount'] = $order['order_amount'] + $order['tax_fee'];
...
elseif ($_REQUEST['step'] == 'ajax_update_cart')
{
    ...
    if (isset($_POST['rec_id']) &&isset($_POST['goods_number']))
    {
        ...
        $sql = "SELECT g.goods_name,g.goods_number, g.goods_weight, g.goods_tax ".
        ...
        $result['subtotal_weight'] = intval($val) * floatval($row['goods_weight']); 
		...
		$result['last_price'] = $cart_goods['total']['last_price'];
		
		$result['total_weight'] = $cart_goods['total']['total_weight'];
		/* 返回购物车信息 */
...
//  $_SESSION['flow_type'] = CART_GENERAL_GOODS;
    ...
    /* 取得商品列表，计算合计 */
    /* 查出类型为普通，团购，一元海购的商品 */
//  $cart_goods = get_cart_goods(array(CART_GENERAL_GOODS,CART_GROUP_BUY_GOODS,CART_HIGO_GOODS));
	
	//将获得的数组分离
	$common_goods = get_cart_goods(array(CART_GENERAL_GOODS));
	$group_goods = get_cart_goods(array(CART_GROUP_BUY_GOODS));
	$higo_goods = get_cart_goods(array(CART_HIGO_GOODS));
	
	$cart_goods['CART_GENERAL_GOODS'] = $common_goods;
	$cart_goods['CART_GROUP_BUY_GOODS'] = $group_goods;
	$cart_goods['CART_HIGO_GOODS']	= $higo_goods;
	
	$smarty->assign('common_goods' , $common_goods);
	$smarty->assign('group_goods'  , $group_goods);
	$smarty->assign('higo_goods'   , $higo_goods);
	...
	foreach($cart_goods as $key=>$value){
		foreach($value['goods_list'] as $k=>$v){
			/* 税金 */
			$sql = 'select goods_tax from'. $GLOBALS['ecs']->table('goods') .'where goods_id='.$v['goods_id'];
			$cart_goods[$key]['goods_list'][$k]['goods_tax'] = $db->getOne($sql);
			/* 重量 */
			$weightsql = 'select goods_weight from'. $GLOBALS['ecs']->table('goods') .'where goods_id='.$v['goods_id'];
			$cart_goods[$key]['goods_list'][$k]['goods_weight'] = $db->getOne($weightsql);
		}
	}
	
	$merge['total_weight'] = $total_weight;
    $smarty->assign('goods_list', $cart_goods['goods_list']);
    $smarty->assign('total', $cart_goods['total']);
	
    //购物车的描述的格式化
    $smarty->assign('shopping_money',         sprintf($_LANG['shopping_money'], $cart_goods['total']['last_price']));
	...
	$merge['total_weight'] = $total_weight;
	//2.向上取整(获得推荐产品所需要的重量)
	$new_weight = ceil($total_weight)-$total_weight;
	//二十元以下的商品
	$twentySql = 'SELECT * from ' . $GLOBALS['ecs']->table('goods') .
				' where is_delete = 0 and is_on_sale = 1 and goods_weight between 0 and ' . $new_weight . 
				' and shop_price<20' .
				' order by goods_weight desc' .
				' limit 8';
	$merge['following_twenty'] = $GLOBALS['db']->getAll($twentySql);
	
	//二十~五十元的商品
	$fiftySql = 'SELECT * from ' . $GLOBALS['ecs']->table('goods') . 
				' where is_delete = 0 and is_on_sale = 1 and goods_weight between 0 and ' . $new_weight . 
				' and shop_price between 20 and 50'.
				' order by goods_weight desc' .
				' limit 8';
	$merge['following_fifty'] = $GLOBALS['db']->getAll($fiftySql);
	
	//五十~一百元的商品
	$hundredSql = 'SELECT * from ' . $GLOBALS['ecs']->table('goods') . 
				' where is_delete = 0 and is_on_sale = 1 and goods_weight between 0 and ' . $new_weight . 
				' and shop_price between 50 and 100'.
				' order by goods_weight desc' .
				' limit 8';
	$merge['following_hundred'] = $GLOBALS['db']->getAll($hundredSql);
	
	//一百元以上的商品
	$moreSql = 'SELECT * from ' . $GLOBALS['ecs']->table('goods') . 
				' where is_delete = 0 and is_on_sale = 1 and goods_weight between 0 and ' . $new_weight . 
				' and shop_price>100'.
				' order by goods_weight desc' .
				' limit 8';
	$merge['following_more'] = $GLOBALS['db']->getAll($moreSql);
...
function preg_pipei($arr){
	preg_match_all('/\d+/',$arr,$arr);
	return implode('.', $arr[0]);
}
function select_tax($gid){
	$sql = 'SELECT goods_tax,shop_price from ecs_goods where goods_id='.$gid;
	$res = $GLOBALS['db']->getRow($sql);
	$tax = $res['goods_tax']*0.01*$res['shop_price'];
	return $tax;
}
